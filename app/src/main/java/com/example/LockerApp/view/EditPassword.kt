...
fun EditPasswordScreen(navController: NavController) {
    ...
    Column(modifier = Modifier.padding(16.dp)) {
        Text(text = "Change Master Password", fontSize = 20.sp, fontWeight = FontWeight.Bold)
        ...
        Button(
            onClick = {
                ...
                val encryptedStoredPassword = sharedPreferences.getString("encrypted_master_password", null)
                ...
                if (encryptedStoredPassword == null || storedIV == null) {
                    ...
                    val (encryptedPassword, iv) = KeystoreManager.encryptData(defaultPassword)

                    // บันทึกใน SharedPreferences
                    sharedPreferences.edit()
                        .putString("encrypted_master_password", Base64.encodeToString(encryptedPassword, Base64.DEFAULT))
                        .putString("encryption_iv", Base64.encodeToString(iv, Base64.DEFAULT))
                        .apply()

                    Toast.makeText(context, "Stored default password", Toast.LENGTH_SHORT).show()
                    return@Button
                }
                ...
                if (oldPassword == decryptedPassword) {
                    ...
                    if (newPassword == confirmPassword) {
                        ...
                        val (encryptedNewPassword, newIv) = KeystoreManager.encryptData(newPassword)

                        // บันทึกรหัสผ่านใหม่ที่เข้ารหัสลงใน SharedPreferences
                        sharedPreferences.edit()
                            .putString("encrypted_master_password", Base64.encodeToString(encryptedNewPassword, Base64.DEFAULT))
                            .putString("encryption_iv", Base64.encodeToString(newIv, Base64.DEFAULT))
                            .apply()
